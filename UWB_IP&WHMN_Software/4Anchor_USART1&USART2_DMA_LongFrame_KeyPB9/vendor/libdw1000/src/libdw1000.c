...
void dwHardReset(dwDevice_t* dev)
{
...
// 更新dw1000的配置，更新之前，先将原有的配置读出来
void dwNewConfiguration(dwDevice_t* dev) {
	dwIdle(dev); // 只有在IDLE模式下，才可以更新配置
...
void dwSetDefaults(dwDevice_t* dev) {
	if(dev->deviceMode == TX_MODE) {
		...
		dwUseExtendedFrameLength(dev, true);  //长帧模式开启
    ...
    dwInterruptOnReceiveTimeout(dev, true);
		...
		dwEnableMode(dev, MODE_LONGDATA_RANGE_ACCURACY);
	}
}
...
void dwTune(dwDevice_t *dev) {
	...
	if(dev->channel == CHANNEL_1 || dev->channel == CHANNEL_2) {
		if(dev->pulseFrequency == TX_PULSE_FREQ_16MHZ) {
			...
			if(dev->smartPower) {
				//writeValueToBytes(txpower, 0x07274767L, LEN_TX_POWER);//default
				
				//Justin add start
				//add power
				//writeValueToBytes(txpower, 0x0D072747L, LEN_TX_POWER);//add 3dB
				writeValueToBytes(txpower, 0x130D0727L, LEN_TX_POWER);//add 6dB
				//writeValueToBytes(txpower, 0x130d0767L, LEN_TX_POWER);//add pre 6dB
				//Justin add end
			} else {
